...
-- ----------------------------------------------------------------------------
/* Exercício 04 - Com base no cpf do cliente calcula 
o total de vendas do cliente.*/
DELIMITER //
create function vendas_cliente(cpf_cliente varchar(11))
returns decimal(10,2)
deterministic
begin
	declare totalVendasCliente decimal(10,2);
    select sum(PRECO * QUANTIDADE) into totalVendasCliente
    from itens_notas_fiscais inf
    join notas_fiscais nf on nf.NUMERO = inf.NUMERO
    where nf.CPF = cpf_cliente;
    return totalVendasCliente;
end //
DELIMITER ;
select vendas_cliente('2600586709')as 'Total de vendas por cliente';
